 ...
 import flash.geom.Rectangle;
    ...
    public class ColorFilter {
        ...
        public static function grayFilter(source:BitmapData):BitmapData {
            ...
            returnBitmapData.applyFilter(returnBitmapData,rect,new Point(0,0),getGrayFilter());*/
        ...
        public static function embossFilter(source:BitmapData, angle:uint = 315):BitmapData{
            ...
            var matrix:Array = [Math.cos(radian + pi4) * 256, Math.cos(radian + 2 * pi4) * 256, Math.cos(radian + 3 * pi4) * 256, Math.cos(radian) * 256, 0, Math.cos(radian + 4 * pi4) * 256, Math.cos(radian - pi4) * 256, Math.cos(radian - 2 * pi4) * 256, Math.cos(radian - 3 * pi4) * 256];
        ...
        public static function spherizeFilter(source:BitmapData):BitmapData {
            ...
            var radian:int, radius:int, offsetX:Number, offsetY:Number, color:int, r:int, g:int, b:int;
            returnBitmapData = source.clone();
        ...
        public static function lightingFilter(source:BitmapData, power:Number = 128, posx:Number = 0.5, posy:Number = 0.5, r:Number = 0):BitmapData {
            ...
            var sr:int = r * r;
            returnBitmapData = source.clone();
            var sd:Number, color:int, g:int, b:int, distance:int, brightness:int;
        ...
        public static function mosaicFilter(source:BitmapData, block:Number = 6):BitmapData {
            //block 1-32
            returnBitmapData = source.clone();
            var sumr:Number, sumg:Number, sumb:Number, product:Number, color:int, r:int, g:int, b:int, br:int, bg:int, bb:int;
        ...
        public static function oilPaintingFilter(source:BitmapData, brushSize:Number = 1, coarseness:Number = 32):BitmapData {
            //brushSize 1-8
            //coarseness 1-255
            var color:uint, gray:uint, r:uint, g:uint, b:uint, a:uint;
        ...
        //亮度
        public static function brightness(p_val:Number):Array {
            var matrix:Array = [1, 0, 0, 0, p_val, 0, 1, 0, 0, p_val, 0, 0, 1, 0, p_val, 0, 0, 0, 1, 0, 0, 0, 0, 0, 1]
            var filter:BitmapFilter = new ColorMatrixFilter(matrix);
            return [filter];
        }
        
        //饱和度
        public static function saturation(rp:Number = 1, gp:Number = 1, bp:Number = 1):Array {
        ...
        //对比度
        public static function contrast(ro:Number = 0, go:Number = 0, bo:Number = 0):Array {
            var matrix:Array = [ro / 127, 0, 0, 0, 0.5 * (127 - ro), 0, go / 127, 0, 0, 0.5 * (127 - go), 0, 0, bo / 127, 0, 0.5 * (127 - bo), 0, 0, 0, 1, 0, 0, 0, 0, 0, 1]
            var filter:BitmapFilter = new ColorMatrixFilter(matrix);
            return [filter];
        }
