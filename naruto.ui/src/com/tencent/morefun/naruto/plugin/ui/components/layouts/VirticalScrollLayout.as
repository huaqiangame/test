package com.tencent.morefun.naruto.plugin.ui.components.layouts 
{
	...
	{		
		...
		{		
			// 开始渲染
			var position:Number = _value * (Math.max(0, _lineCount - _row)) * (_itemHeight + _vgap) / 100;
		...
		override protected function swapItemOrder(scrollingDown:Boolean):void
		{
			...
			if (scrollingDown)
			{
				for (index = 0; index < _column; index++)
				{
					...
					(lastItem.parent) && (lastItem.parent.setChildIndex(lastItem,0));
				}
			}
			else
			{
				for (index = 0; index < _column; index++)
				{
					...
					(firstItem.parent) && (firstItem.parent.setChildIndex(firstItem,firstItem.parent.numChildren-1));
				}
			}
		...
		override public function scrollTo(dataIndex:int):void 
		{
			...
			_forceUpdate = true;
			this.value = _lineCount > _row? 100 * lineIndex / (_lineCount - _row) : 0;
		}
		...
		override public function set dataProvider(value:Array):void 
		{
			...
			this.value = _lineCount > _row? 100 * _lineIndex / (_lineCount - _row) : 0;
		}		
	}
}