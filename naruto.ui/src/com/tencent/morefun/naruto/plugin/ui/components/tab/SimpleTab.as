package com.tencent.morefun.naruto.plugin.ui.components.tab 
{
	...
	public class SimpleTab extends Sprite
	{
		...
		private var _sandwich:DisplayObject;
		 ...
		 * 刷新显示深度
		 */
		public function depthRefresh():void
		{
			var list:Vector.<TabItemRender> = _items.concat();
			list.sort(function(t1:TabItemRender, t2:TabItemRender):int
			{
				if (t1.depth != t2.depth) return t1.depth > t2.depth? 1 : -1;
				return (t1.x + t1.y) > (t2.x + t2.y)? 1 : -1; // 坐标大的页签排在上面
			});
			
			for (var i:int = 0; i < list.length; i++) setChildIndex(list[i], i);
			
			if (_sandwich)
			{
				setChildIndex(_sandwich, numChildren - 1);
			}
			
			// 被选中的页签显示在最上面
			setChildIndex(_selectedItem, numChildren - 1);
		}
		...
		private function layoutUpdate(forceUpdate:Boolean = true):void
		{
			...
			if (_horizontal)
			{
				_width = position - _gap;
				_height = item.height;
			}
			else
			{
				_width = item.width;
				_height = position - _gap;
			}
		}
		...
		public function dispose():void
		{
			...
			if (_sandwich)
			{
				_sandwich.parent && _sandwich.parent.removeChild(_sandwich);
				_sandwich = null;
			}
		}
		...
		/**
		 * 页签夹心层
		 * @usage 需要让页签夹在视图的两边的情况下使用
		 */
		public function get sandwich():DisplayObject { return _sandwich; }
		public function set sandwich(value:DisplayObject):void 
		{
			if (_sandwich && _sandwich != value)
			{
				_sandwich.parent && _sandwich.parent.removeChild(_sandwich);
			}
