...
{	
	...
	public class ScrollLayout extends Sprite implements IComponent
	{
		...
		protected function resetView():void
		{
			...
			_scrollRect = new Rectangle();
//			_scrollRect.width = _column * (_itemWidth + _hgap) - (_hgap>0 ? _hgap : 0);   //当gap为负数的时候，实际上是想裁剪掉一些，所以即使是最后一个，也同样裁剪
//			_scrollRect.height = _row * (_itemHeight + _vgap) - (_vgap>0 ? _vgap : 0);    //当gap为负数的时候，实际上是想裁剪掉一些，所以即使是最后一个，也同样裁剪
			_scrollRect.width = (_column - 1) * (_itemWidth + _hgap) + _itemWidth;   //当gap为负数的时候，最后一个不用裁剪。
			_scrollRect.height = (_row - 1) * (_itemHeight + _vgap) + _itemHeight;   //当gap为负数的时候，最后一个不用裁剪。
		 ...
		 * 获取当前鼠标下的渲染器
		 */
		public function getItemUnderMouse():IRender
		{
			var bounds:Rectangle;
			for each(var item:RenderWrapper in _items)
			{
				bounds = item.getBounds(this);
				if (bounds.contains(mouseX, mouseY)) return item.target;
			}
		...
		{
			return index < _items.length? _items[index].target : null;
		}
		 ...
		 * @param	data	数据
		 */
		public function getItemWith(data:Object):IRender
		{
			if (!data) return null;
			for each(var item:RenderWrapper in _items)
			{
				if (item.data == data) return item.target;
			}
		...
		public function dispose():void
		{
			while (_items && _items.length)
			{
				(_items.pop() as RenderWrapper).dispose();
			}
		...
		public function set value(value:Number):void
		{
			...
			if (value == _value && !_forceUpdate) return;
