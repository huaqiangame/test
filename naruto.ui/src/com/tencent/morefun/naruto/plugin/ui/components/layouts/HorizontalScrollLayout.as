package com.tencent.morefun.naruto.plugin.ui.components.layouts
{
	 ...
	 * @author larryhou
	 */
	public class HorizontalScrollLayout extends ScrollLayout
	{	
		
		/**
		 * 构造函数
		 * create a [HorizontalScrollLayout] object
		 * @param	rowCount		每页显示的行数
		 * @param	columnCount		每页显示的列数
		 * @param	hgap			水平方向间隔
		 * @param	vgap			垂直方向间隔
		 */
		public function HorizontalScrollLayout(rowCount:int, columnCount:int = 1, hgap:int = 5, vgap:int = 5)
		{			
		...
		{		
			// 开始渲染	
			var position:Number = _value * (Math.max(0, _lineCount - _column)) * (_itemWidth + _hgap) / 100;
		...
		override public function scrollTo(dataIndex:int):void 
		{
			...
			_forceUpdate = true;
			this.value = _lineCount > _column? 100 * lineIndex / (_lineCount - _column) : 0;
		}
		...
		override public function set dataProvider(value:Array):void 
		{
			...
			this.value = _lineCount > _column? 100 * _lineIndex / (_lineCount - _column) : 0;
		}
	}
